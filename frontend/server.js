...
function getAccountDir(account) {
  ...
  const safe = account.replace(/[^a-zA-Z0-9_.\-]/g, '');
  if (!safe) throw new Error('Invalid account name');
  return path.join(SAVED_POSTS_BASE, safe);
}
...
app.use('/media', express.static(SAVED_POSTS_BASE));
  ...
  try {
    ...
    if (fs.existsSync(SAVED_POSTS_BASE)) {
      const dirs = fs.readdirSync(SAVED_POSTS_BASE, { withFileTypes: true })
        .filter(d => d.isDirectory() && fs.existsSync(path.join(SAVED_POSTS_BASE, d.name, 'posts-index.json')))
        .map(d => d.name)
        .sort();
...
app.post('/api/duplicates/auto-clean', (req, res) => {
  ...
  try {
    ...
    for (const dup of exactDuplicates) {
      ...
      deletePostById(account, dup.postIds[1]);
...
app.post('/api/duplicates/merge', (req, res) => {
  ...
  try {
    ...
    deletePostById(account, deleteId);
...
// Helper function to delete post by ID within an account
function deletePostById(account, id) {
  const postsDir = getAccountDir(account);
